...
def setup_matrices(path, label, groups, usable_features=None, drop_group_as_feature=False,
    ...
    for i in range(X_in.shape[1]):  # iterate over each column of the numpy matrix
        ...
        # NOTE: corrected to df[usable_features].columns[i] from df.columns[i]
        if X_in[0, i] is not None and (type(X_in[0, i]) == str or df[usable_features].columns[i] in categorical_columns):
            try:
                lb = LabelBinarizer()
