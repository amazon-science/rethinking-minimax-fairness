...
Modified to add more classification models
"""
...
def do_learning(X, y, numsteps, grouplabels, a=1, b=0.5,  equal_error=False, scale_eta_by_label_range=True,
                ...
                model_class=None, classification_models=(), return_models=True, warm_start=True, warm_start_weight=0.95):
    """
    ...
    :param warm_start: initialize group weights as 0.95 to group with max error and 0.05 equally distributed among rest
    """
    ...
    if error_type.endswith('Log-Loss') and model_type not in ['LogisticRegression', 'MLPClassifier', 'TabularMLPClassifier', 'LogisticRegressionSGD']:
        warnings.warn('WARNING: Log-Loss error type is only supported for Logistic Regression.')
    ...
    for t in range(1, numsteps):
        ...
        if (('KitchenSinkLogistic' in model_type) or ('LogisticRegressionSGDdim' in model_type))\
            and (type(model_class).__name__=='Pipeline'):
            fit_params = {model_class.steps[-1][0] + '__sample_weight': avg_sampleweights}
            modelhat = model_class.fit(X_train, y_train, **fit_params)
        elif model_type in ['MLPClassifier','TabularMLPClassifier']:
            modelhat = model_class.fit(X_train, y_train, sampleweights=avg_sampleweights, n_epochs=n_epochs)
        elif model_type=='PairedRegressionClassifier':
            modelhat = model_class.fit(X_train, y_train, avg_sampleweights)
        else:
        ...
        for i in range(num_group_types):
            ...
            if relaxed:  # Projected Gradient descent
                ...
                if equal_error:  # GD where errors are pushed to mean error
                    ...
                    if t==1 and warm_start:
                        # Assign 0.9 to group with highest error and distribute 0.1 equally among remaining groups
                        groupweights[i][t] = np.ones_like(groupweights[i][t - 1]) * (1-warm_start_weight)/(numgroups[i]-1)
                        max_group_index = np.argmax(grouperrs[i][t])
                        groupweights[i][t][max_group_index] = warm_start_weight
                    else:
    ...
    if display_plots or save_plots:
        ...
        model_string = f'\n {model_type}' + loss_string + (' for Equal-Error' if equal_error else '') + \
                       (f'\n Gamma={gamma}' if relaxed else '') + \
                        (f'\n warm start weight on max error group {warm_start_weight}' if warm_start else '')
...
def rescale_feature_matrix(X):
    ...
    for col in range(X.shape[1]):
        if max(abs(X[:, col]))!=0.0:  # skip rescaling constant 0 features
