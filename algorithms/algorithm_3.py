...
Only one modification in line 133 `L[L==0] = 1`
Handle divide by zero error by putting 1 in L
"""
...
def algorithm_3(X, label, protected_attribute, number_of_groups,
    ...
    unique_attributes = np.unique(protected_attribute)
    y_train_split = [torch.Tensor(label[protected_attribute == i]).long() for i in
                     unique_attributes]
    x_train_split = [torch.Tensor(X[protected_attribute == i]) for i in unique_attributes]
    ...
    return loss_history_per_group, error_history_per_group, loss_history_overall, \
           error_history_overall, differences_in_iterates, number_of_computations, keep_weights, f_av
